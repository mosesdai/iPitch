...
| 组件 | 推荐服务 | iPitch 用途 |
|------|----------|-------------|
| 静态 UI | **GitHub Pages**、Netlify、Vercel、Cloudflare Pages | 托管 `ui/ipitch-studio.html` |
...
**静态站（GitHub Pages 示例）**  
1. 推送 `_ipitch` 到 GitHub  
2. Settings → Pages → Source: 选分支与目录  
3. 访问 `https://<org>.github.io/<repo>/ui/ipitch-studio.html`  
...
用户浏览器
    │
    ├─► GitHub Pages  ──►  ui/ipitch-studio.html（静态，无密钥）
    │
...
- 异步工作流（能睡着等结果吗）：`docs/ASYNC_WORKFLOW.md`  
- 变更记录：`CHANGELOG.md`  
- 主 README：`README.md`
